...
def _record_for(path: Path, root: Path, result: PolishResult) -> dict[str, Any]:
    ...
    return {
        "file": path.relative_to(root).as_posix(), "mode": "polish", "preset": None,
        "assurance": result.assurance, "status": _status_for(result),
        "source_text": result.source, "candidate_text": result.candidate, "final_text": result.final_text,
        "used_fallback": result.used_source_fallback, "safe_for_automatic_use": result.safe_for_automatic_use,
        ...
        "repair_success": result.repair_success, "repair_failure_reason": result.repair_failure_reason,
        "verifier_used": verification.verifier_used if verification is not None else False,
        "verification_skip_reason": (
            verification.verifier_skip_reason if verification is not None else (
                "diagnostics_no_change" if result.generation_skipped_by_diagnostics
        ...
        "semantic_deltas": [delta.to_dict() for delta in verification.semantic_deltas] if verification is not None else [],
        "diagnostics_before": result.diagnostics_before.to_dict() if result.diagnostics_before is not None else None,
        "diagnostics_after": None, "rewrite_token_usage": result.rewrite_token_usage,
        "verification_token_usage": verification.token_usage if verification is not None else None,
        "notes": result.notes, "human_review": {"category": None, "notes": None},
    }


def collect_dogfood(
    *, input_dir: str | Path, output_dir: str | Path, rewrite_provider: RewriteProvider,
    ...
    files = sorted(path for path in source_root.rglob("*") if path.is_file() and path.suffix.casefold() in SUPPORTED_SUFFIXES)
    if not files:
    ...
    for path in files:
        ...
        result = polish_text(
            source=source, rewrite_provider=rewrite_provider, verifier_provider=verifier_provider,
            assurance=assurance, run_diagnostics=run_diagnostics,
        )
    ...
    diagnostics_note = (
        ...
        "Pre-generation diagnostics were disabled for this run so rewrite/verifier coverage is preserved for semantic calibration."
    )
    ...
    summary = {
        "mode": "polish", "preset": None, "assurance": assurance,
        "diagnostics_enabled": run_diagnostics, "sample_count": len(records),
        ...
        "generation_skipped_by_diagnostics": sum(1 for item in records if item["generation_skipped_by_diagnostics"]),
        "repair": {
            "cases_attempted": len(repair_attempted), "total_attempts": total_repair_attempts,
            "successes": repair_successes,
            "success_rate": repair_successes / len(repair_attempted) if repair_attempted else 0.0,
            "average_attempts_per_repair": total_repair_attempts / len(repair_attempted) if repair_attempted else 0.0,
        },
