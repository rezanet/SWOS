...
def stable_path(path: Path) -> str:
    """Keep generated review manifests independent of runner-local temp roots."""
    return path.name
...
def blank_human_fields(row: dict[str, Any], line_no: int) -> None:
    ...
    if any(
        adjudication.get(key) not in (None, "") for key in ("adjudicator_id", "label", "rationale")
    ):
...
def prepare_row(
    ...
    if any(key in row for key in ("label", "relation", "retrieval_intent")):
        raise ValueError(
            f"line {line_no}: unlabelled packet exposes a forbidden truth/intent field"
        )
    ...
    blind = {
        ...
        "integrity": {"record_sha256": None, "signed_or_immutable_external_record_uri": None},
        "blindness": {
...
def main() -> int:
    ...
    parser.add_argument("--source-rights-review", type=Path)
    ...
    try:
            ...
            adjudication_rows.append(
                {
                    ...
                    "adjudication": {
                        ...
                        "conflict_declaration": None,
                        "reviewed_at": None,
                        "annotation_agreement_before_adjudication": None,
                        "final_label": None,
                        "allowed_labels": list(ALLOWED_LABELS),
                        "rationale": None,
                        "disposition": None,
        ...
        if len(blind_rows) < 6000:
            raise ValueError(
                f"citation release floor requires at least 6000 unique candidate pairs; got {len(blind_rows)}"
            )
        out = args.output_dir.resolve()
        source_rights_review = (
            args.source_rights_review.resolve() if args.source_rights_review else None
        )
        if source_rights_review is not None and not source_rights_review.is_file():
            raise ValueError(f"source rights review does not exist: {source_rights_review}")
        ...
        manifest = {
            ...
            "source_pairs_path": stable_path(args.pairs.resolve()),
            ...
            "worksets": {
                "A": {
                    "path": "annotator-A.jsonl",
                    "sha256": sha256_file(out / "annotator-A.jsonl"),
                },
                "B": {
                    "path": "annotator-B.jsonl",
                    "sha256": sha256_file(out / "annotator-B.jsonl"),
                },
                "adjudication": {
                    "path": "adjudication-bindings.jsonl",
                    "sha256": sha256_file(out / "adjudication-bindings.jsonl"),
                },
            },
            "source_rights_review": (
                {
                    "path": os.path.relpath(source_rights_review, out),
                    "sha256": sha256_file(source_rights_review),
                }
                if source_rights_review is not None
                else None
            ),
