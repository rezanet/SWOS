...
def main() -> int:
    parser = argparse.ArgumentParser(prog="swos", description="Autonomous governed SWOS research-writing runtime")
    sub = parser.add_subparsers(dest="command", required=True)
    write = sub.add_parser("research-write", help="Run research -> evidence -> argument -> draft -> verify -> review -> audit")
    ...
    if args.command == "research-write":
        ...
        if args.as_json:
            ...
            print(f"{outcome.status}: {outcome.output_dir} ({outcome.article_word_count} body words)")
