...
def _run_probe(sandbox: Path, probe: str, timeout_seconds: float) -> dict[str, Any]:
    ...
    try:
        ...
        stdout_bytes, stdout_sha256 = _output_metadata(exc.stdout)
        stderr_bytes, stderr_sha256 = _output_metadata(exc.stderr)
        return {
            "status": "error",
            "returncode": None,
            "timed_out": True,
    ...
    if result.returncode == 0:
        ...
        status = "error"
    stdout_bytes, stdout_sha256 = _output_metadata(result.stdout)
    stderr_bytes, stderr_sha256 = _output_metadata(result.stderr)
    return {
        "status": status,
...
def _run_mutations(timeout_seconds: float) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    with tempfile.TemporaryDirectory(prefix="swos-mutation-") as temporary:
        ...
        for probe in sorted({spec.probe for spec in MUTANTS}):
            ...
            baseline[probe] = _run_probe(sandbox, probe, timeout_seconds)
        for index, spec in enumerate(MUTANTS):
            ...
            try:
                ...
                probe_result = _run_probe(sandbox, spec.probe, timeout_seconds)
...
def run_mutation_checks(*, timeout_seconds: float = 60.0) -> dict[str, Any]:
    if not 1 <= timeout_seconds <= 300:
        raise ValueError("timeout_seconds must be between 1 and 300")
    baseline, mutants = _run_mutations(timeout_seconds)
    ...
    return {
        ...
        "status": "passed" if len(killed) == len(MUTANTS) else "failed",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        report = run_mutation_checks(timeout_seconds=args.timeout_seconds)
