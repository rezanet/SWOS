...
def _metric(
    ...
    *,
    ...
    passed = bool(denominator) and (
        (value >= threshold) if direction == "higher_is_better" else (value <= threshold)
    )
    result = {
        "metric": name,
        "numerator": numerator,
...
def _request_from_case(
    case: Mapping[str, Any], assets: Sequence[MediaAssetRecord]
) -> ImageAnalysisRequest:
    ...
    return ImageAnalysisRequest(
        ...
        resource_limits=dict(
            ...
            or {"max_assets": 8, "max_observations": 64, "max_seconds": 60}
        ),
    ...
    for observation in result.get("observations", []):
        ...
        if isinstance(normalized, (list, tuple)) and len(normalized) == 4:
            try:
                regions.add(
                    (
                        str(observation.get("asset_id") or ""),
                        tuple(int(value) for value in normalized),
                    )
                )
...
def _expected_regions(expected: Mapping[str, Any]) -> set[tuple[str, tuple[int, ...]]]:
    ...
    for item in values:
        ...
        if isinstance(normalized, (list, tuple)) and len(normalized) == 4:
            try:
                regions.add(
                    (str(item.get("asset_id") or ""), tuple(int(value) for value in normalized))
                )
            except (TypeError, ValueError):
                continue
    return regions
...
def _cross_modal_counts(case: Mapping[str, Any], result: Mapping[str, Any]) -> dict[str, int]:
    ...
    for pair in pairs:
        ...
        expected_supported = bool(
            pair.get(
                "expected_supported",
                pair.get("supported", pair.get("expected_status") == "supported"),
            )
        )
...
def _safety_counts(case: Mapping[str, Any], result: Mapping[str, Any]) -> dict[str, int]:
    ...
    counts = {
        "false_originality_detected": 0,
        "false_originality_total": 0,
        "over_association_detected": 0,
        "over_association_total": 0,
    }
    observations = result.get("observations", [])
    textual = case.get("textual_evidence") or []
    for item in cases:
...
def _case_evaluation(
    case: Mapping[str, Any], provider: Any, repetitions: int
) -> tuple[dict[str, Any], dict[str, int]]:
    assets = tuple(
        _asset_from_case(item) for item in case.get("assets", []) if isinstance(item, Mapping)
    )
...
def evaluate_multimodal_cases(
    ...
    *,
    ...
    totals = {
        ...
        "region_numerator": 0,
        "region_denominator": 0,
        "grounding_numerator": 0,
        "grounding_denominator": 0,
        "false_region_numerator": 0,
        "false_region_denominator": 0,
        "stability_numerator": 0,
    ...
    metrics: dict[str, Any] = {
        "agreement": _metric(
            "agreement", totals["agreement_numerator"], totals["agreement_denominator"], 1.0
        ),
        "region_hit": _metric(
            "region_hit", totals["region_numerator"], totals["region_denominator"], 0.90
        ),
        "visual_grounding_coverage": _metric(
            "visual_grounding_coverage",
            totals["grounding_numerator"],
            totals["grounding_denominator"],
            1.0,
        ),
        "false_region": _metric(
            "false_region",
            totals["false_region_numerator"],
            totals["false_region_denominator"],
            0.02,
            direction="lower_is_better",
        ),
        "cross_modal_precision": _metric(
            "cross_modal_precision", totals["cross_tp"], precision_denominator, 0.98
        ),
        "cross_modal_recall": _metric(
            "cross_modal_recall", totals["cross_tp"], recall_denominator, 0.90
        ),
        "cross_modal_f1": _metric(
            "cross_modal_f1",
            2 * totals["cross_tp"],
            f1_denominator,
            0.94,
            extra={
                "true_positive": totals["cross_tp"],
                "false_positive": totals["cross_fp"],
                "false_negative": totals["cross_fn"],
            },
        ),
        "false_originality_detection": _metric(
            "false_originality_detection",
            totals["false_originality_detected"],
            totals["false_originality_total"],
            1.0,
        ),
        "over_association_detection": _metric(
            "over_association_detection",
            totals["over_association_detected"],
            totals["over_association_total"],
            1.0,
        ),
        "valid_reviewed_accessibility_completeness": _metric(
            "valid_reviewed_accessibility_completeness",
            totals["valid_accessibility"],
            totals["required_accessibility"],
            1.0,
        ),
        "stability": _metric(
            "stability", totals["stability_numerator"], totals["stability_denominator"], 1.0
        ),
    }
    object_ids = {
        str(case.get("object", {}).get("object_id") or asset.get("object_id") or "")
        for case in cases
    ...
    metrics["object_minimum"] = (
        _metric("object_minimum", len(object_ids), required_objects, 1.0)
        if required_objects
        else {
            "metric": "object_minimum",
            "numerator": len(object_ids),
            "denominator": 0,
            "value": None,
            "threshold": 0,
    ...
    metrics["rendition_minimum"] = (
        _metric("rendition_minimum", len(asset_ids), required_renditions, 1.0)
        if required_renditions
        else {
            "metric": "rendition_minimum",
            "numerator": len(asset_ids),
            "denominator": 0,
            "value": None,
            "threshold": 0,
            "status": "not_applicable",
    ...
    blocking_metrics = [
        ...
        if value.get("status") != "not_applicable" and not value.get("passed")
    ]
    return {
        ...
        "limitations": ["regression baseline not supplied"]
        if "regression" in blocking_metrics
        else [],
        "input_digest": canonical_digest(
            {"cases": list(cases), "repetitions": repetitions, "minimums": minimums}
        ),
    }


def _not_run_report(
    manifest: Mapping[str, Any], *, reason: str, source_sha: str | None = None
) -> dict[str, Any]:
    return {
        "schema_version": "2.0.0",
        "status": "NOT_RUN",
    ...
    *,
    ...
    try:
        ...
        report = _not_run_report(
            {}, reason=f"NOT_RUN: manifest unavailable: {exc}", source_sha=source_sha
        )
    ...
    if str(manifest.get("status") or "").lower() != "ready" or not manifest.get("cases"):
        report = _not_run_report(
            manifest,
            reason=str(manifest.get("reason") or "NOT_RUN: a locked reviewed corpus is required"),
    ...
    if require_live and provider is None:
        report = _not_run_report(
            manifest,
            reason="NOT_RUN: live provider capability is not supplied",
            source_sha=source_sha,
        )
        _write_json(artifact_root / "multimodal-evaluation-report.json", report)
        return report
    selected_provider = provider or DeterministicFakeImageProvider()
    report = evaluate_multimodal_cases(
        manifest.get("cases") or [],
        provider=selected_provider,
        repetitions=repetitions,
        source_sha=source_sha,
        minimums=manifest.get("required_counts"),
    )
    _write_json(
        artifact_root / "raw-case-results.json",
        {
            "schema_version": "2.0.0",
            "source_sha": report["source_sha"],
            "cases": report["raw_case_results"],
        },
    )
    report["artifact_identities"] = {
        "raw-case-results.json": _json_digest(
            {
                "schema_version": "2.0.0",
                "source_sha": report["source_sha"],
                "cases": report["raw_case_results"],
        ...
        "manifest": _json_digest(manifest),
    }
    _write_json(artifact_root / "multimodal-evaluation-report.json", report)
    return report
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = run_evals(
        ...
        require_live=args.live and args.provider == "openai",
    )
    return (
        0
        if report.get("gate_result") == "pass"
        else (2 if report.get("status") == "NOT_RUN" else 1)
    )
