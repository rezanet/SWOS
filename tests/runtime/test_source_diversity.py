...
def source(index: int, *, provider: str = "provider-a", owner: str | None = None, stance: str = "support") -> dict:
    return {
        "source_id": f"s-{index}", "title": f"Work {index}", "url": f"https://example.org/{index}",
        "provider": provider, "doi": f"10.1000/{index}", "publisher": owner or f"Owner {index}",
        "venue": f"Venue {index}", "region": "AU" if index % 2 else "UK", "language": "en",
        "period": "2020s", "methodology": "empirical" if index % 2 else "review",
        "source_type": "article", "access_mode": "open", "stance": stance,
...
class SourceDiversityTests(unittest.TestCase):
    def test_family_identity_is_invariant_to_order_provider_and_mirror(self) -> None:
        one = source(1)
        mirror = {**one, "source_id": "mirror", "url": "https://mirror.invalid/one", "provider": "provider-b"}
    ...
    def test_unknown_metadata_does_not_improve_score_and_concentration_uses_claim_exposure(self) -> None:
        ...
        requirements = DiversityRequirement(
            requirement_id="req-1", dimensions=tuple(DIMENSIONS), min_family_count=5,
            required_strata={"stance": ["support", "counter"]}, counter_position_required=True,
        )
        admitted = [{"source_id": item["source_id"]} for item in sources]
        report = measure_source_diversity(families=families, admitted_claims=admitted, requirements=requirements)
    ...
    def test_fewer_than_three_families_blocks_and_narrow_exception_discloses_limit(self) -> None:
        families = canonicalize_source_families([source(1), source(1, provider="other")], FamilyIdentityPolicy())
        requirements = DiversityRequirement(requirement_id="req-2", dimensions=("publisher",), min_family_count=5)
        report = measure_source_diversity(
            families=families,
            admitted_claims=[{"source_id": "s-1"}],
            requirements=requirements,
            exception={"sdl_decision_id": "dec-1", "rationale": "field is narrow", "scope": "question", "expires_at": "2099-01-01T00:00:00Z"},
        )
