...
class CapabilityPromotionTests(unittest.TestCase):
    def _evidence(self, **changes):
        value = {"source_sha": "a" * 40, "artifact_digest": "b" * 64, "case_ids": ["c1", "c2"], "provider": "fake", "model": "m", "config_digest": "d" * 64, "prompt_digest": "e" * 64, "seed": 7, "draw_digest": "f" * 64, "live_exact_head": True, "human_quorum": True, "role_separation": True, "safety_regressions": [], "rollback_tested": True, "pack_only_fallback": True}
        value.update(changes)
        return value

    def test_default_off_and_mismatched_heads_are_disabled(self) -> None:
        assessment = assess_promotion(capability="image_analysis", pack="art_history", stage="art_history_agent", baseline=self._evidence(), candidate=self._evidence(source_sha="c" * 40), policy={"minimum_improvement": 0.08})
        self.assertEqual("disabled", assessment.status)
        self.assertFalse(assessment.eligible)

    def test_improvement_ci_live_and_approval_are_required(self) -> None:
        assessment = assess_promotion(capability="image_analysis", pack="art_history", stage="art_history_agent", baseline=self._evidence(metric=0.70), candidate=self._evidence(metric=0.80), policy={"minimum_improvement": 0.08, "lower_confidence_bound_minimum": 0.0})
        self.assertEqual("disabled", assessment.status)
        decision = commit_promotion(assessment, {"disposition": "approved", "approver_id": "human-1"})
    ...
    def test_eligible_pair_requires_artifact_identity_and_can_roll_back(self) -> None:
        ...
        rolled_back = __import__("swos_runtime.image_analysis", fromlist=["rollback_promotion"]).rollback_promotion(decision, reason="safety regression")
