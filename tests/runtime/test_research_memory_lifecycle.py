...
class ResearchMemoryLifecycleTests(unittest.TestCase):
    ...
    def test_write_confirm_correct_supersede_contradict_expire_and_delete_are_append_only(
        self,
    ) -> None:
        ...
        self._commit(
            RPMOperation.contradiction_open(self.scope, "item-3", reason="counter-evidence")
        )
    ...
    def test_exact_expiry_excludes_normal_reads_but_governance_reads_are_receipted(self) -> None:
        self._commit(
            RPMOperation.write(self.scope, self._candidate(expiry="2026-09-01T00:00:00Z")),
            as_of="2026-08-31T00:00:00Z",
        )
        normal = self.service.query(
            self.scope,
            MemoryQuery(),
        ...
        exceptional = self.service.query(
            self.scope,
            MemoryQuery(),
    ...
    def test_project_retirement_unbinding_and_programme_closure_preserve_history(self) -> None:
        ...
        with self.assertRaises(Exception):
            self.service.assess_operation(
                self.scope, RPMOperation.write(self.scope, self._candidate("after-retire"))
            )
