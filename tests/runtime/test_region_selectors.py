...
class RegionSelectorTests(unittest.TestCase):
    def setUp(self) -> None:
        self.asset = MediaAssetRecord(asset_id="a", object_id="o", role="detail", mime_type="image/jpeg", byte_size=10, width=1000, height=500, byte_digest="a" * 64, acquisition_uri="https://example.org/a")

    def test_pixel_and_percent_selectors_normalize_deterministically(self) -> None:
        pixel = normalize_selector(RegionSelector("iiif_pixel", "10,20,300,100", "a" * 64), self.asset)
        percent = normalize_selector(RegionSelector("iiif_percent", "pct:10,20,30,40", "a" * 64), self.asset)
        self.assertEqual((10, 20, 300, 100), pixel.normalized)
        self.assertEqual((100, 100, 300, 200), percent.normalized)
        self.assertEqual(pixel.to_dict(), normalize_selector(RegionSelector("iiif_pixel", "10,20,300,100", "a" * 64), self.asset).to_dict())
