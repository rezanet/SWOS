...
class RPMExchangeTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        candidate = MemoryCandidate("item-1", "finding", "ref:item-1", 0.8, DataClassification.PUBLIC, "owner", "2099-01-01T00:00:00Z", True, ("epg:item-1",), "sdl:item-1", parent_digest="b" * 64, origin="fixture")
    ...
    def test_export_inspect_commit_is_idempotent_and_preserves_origin_mapping(self) -> None:
        ...
        approval = HumanApproval("export-approval", "reviewer", "memory_owner", "2026-09-01T00:00:00Z", "a" * 64, "b" * 64, "sdl:export", "approved", "export")
        out = self.root / "bundle"
        receipt = exchange.export_bundle(self.scope, ExportSelection(), approval, BundleLimits(), out)
        inspection = exchange.inspect_import(out, destination=self.scope, limits=BundleLimits())
        committed = exchange.commit_import(inspection.inspection_id, inspection.inspection_digest, approval)
        self.assertEqual(receipt.origin_scope, committed.origin_scope)
        self.assertEqual("noop", exchange.commit_import(inspection.inspection_id, inspection.inspection_digest, approval).status)
    ...
    def test_zip_slip_duplicate_path_and_decompression_limit_are_rejected(self) -> None:
        exchange = RPMExchange(self.service)
        with self.assertRaises(Exception):
            exchange.inspect_import(self.root / "../evil.zip", destination=self.scope, limits=BundleLimits(max_bytes=1))
