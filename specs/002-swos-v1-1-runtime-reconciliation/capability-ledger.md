...
| V11-RERANK-001 | Cross-encoder reranker reference implementation before capability expansion | Retrieval | specified | Nearby partial: `swos_runtime/llm.py::OpenAIStageProvider.rerank` performs model joint scoring; `swos_runtime/governance.py::cross_encoder_executed` checks only a capability marker | Broker normalization test only | Demonstration: —; certification: — | Implement an explicit cross-encoder path, identity/provenance, deterministic fixtures and fail-closed fallback in retrieval slice. |
| V11-CITE-001 | Citation identity and bibliographic metadata | Retrieval | tested | `swos_runtime/models.py::SourceRecord`; OpenAlex/Crossref parsing; source register output | OpenAlex and Crossref adapter tests; finalizer complete-run tests | Demonstration: deterministic run artifact; certification: — | Extend normalization and negative metadata cases; preserve existing fields. |
| V11-CITE-002 | Retraction-status checking before support/release | Retrieval | specified | Partial only: generated EPG hard-codes `retraction_status: not_checked` | — | Demonstration: —; certification: — | Add resolver, recorded source/result/time/provenance and fail-closed policy tests. |
| V11-CITE-003 | Licence and source-rights checking before store/export | Retrieval | specified | Partial only: generated EPG reports `licence: source-controlled`, `access_status: unknown`, `redistribution_allowed: false` | — | Demonstration: —; certification: — | Add licence/rights resolver and policy binding with deterministic allow/deny/unknown fixtures. |
...
| V11-CITE-005 | Claim-support classification with fail-closed decisions | Retrieval | tested | `CapabilityBroker.citation_support_audit`; `WorkOrderRun._validate_citation_audit`; finalizer accepts only `directly_supports` | runtime bad-evidence/finalizer tests and work-order validation suite | Demonstration: complete injected run; certification: — | Add deterministic support fixtures and explicit direct/partial/contextual/contradictory/failed mapping. Trained classifier remains `v2.0`. |
...
| V11-CLI-001 | Working end-to-end SWOS CLI | Reconciliation | implemented | `swos_runtime/cli.py::main` exposes `research-write`, work-order and finalization commands; `pyproject.toml` entry point | No deterministic runtime CLI test | Demonstration: manual workflows define use but no checked-in execution evidence; certification: — | Add subprocess/smoke tests without provider calls in the next applicable implementation slice. |
...
PR #41 delivered substantial reusable runtime substrate. The highest-priority
verified implementation gap remains the cross-encoder reranker. Public scholarly
adapters, quotation containment, claim-support gating, orchestration and audit
artifact generation already exist and must be extended rather than duplicated.
The complete `v1.1` programme is not yet demonstrated or certified.
